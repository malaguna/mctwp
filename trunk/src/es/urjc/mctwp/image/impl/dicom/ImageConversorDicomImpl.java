...
/**
 * Options to convert to DICOM "-c dicom"
 * @author miguel
 *
 */
public class ImageConversorDicomImpl implements ImageConversor {
	...
	public List<File> toDicom(Image image, File outputDir) throws ImageException{
		...
		if(image == null)
			...
			error = "Destination [" + outputDir.getAbsolutePath() + "] is not a directory";
		else if(image instanceof SingleImageDicomImpl)
	...
	public PatientInfo getPatientInfo(Image image) throws ImageException {
		...
		if(image == null)
			...
			sidi = (SingleImageDicomImpl)((SeriesImageDicomImpl)image).getImages().get(0);
		else if(image instanceof SingleImageDicomImpl)
	...
	/**
	 * Obtains an XML node representation from the first singleImage 
	 * of the series
	 * 
	 * @param series
	 * @return
	 * @throws ImageException
	 */
	private Node toXml(SeriesImage series) throws ImageException {
		Node result = null;
	...
	/**
	 * Obtains a DICOM representation for each single image into
	 * serie.
	 * 
	 * @param series
	 * @return
	 * @throws ImageException
	 */
	private List<File> toDicom(SeriesImage series, File outputDir) throws ImageException {
		List<File> result = null;
		List<File> aux = null;
	...
	private List<File> toDicom(SingleImage single, File outputDir) throws ImageException {
		...
		try{
			...
			File dest = new File(FilenameUtils.concat(outputDir.getAbsolutePath(), single.getId() + "." + ext));
	 ...
	 * 
	 * @param command to exec into the server
	 * @param wait time to waoit
	 ...
	 * 
	 * @param file original file to scale
	 * @throws Exception
	 */
	private void scaleThumbnail(File file) throws Exception{
		...
		try{
			if(file != null){
			    ...
			    graphics2D.drawImage(pngImage, 0, 0, thumbWidth, thumbHeight, null);
	...
	/**
	 * Get the XSL transformer to convert dicom into XML. If there is an 
	 * xslt specified, it is used.
	 * 
	...
	private TransformerHandler getTransformerHandler() throws Exception {
		...
		return (sc == null)?stf.newTransformerHandler():stf.newTransformerHandler(sc);
	}
}
